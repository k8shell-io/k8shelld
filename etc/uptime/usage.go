package main

import (
	"fmt"
	"os"
	"strconv"
	"strings"
	"time"
)

// Paths to cgroups v2 files
const (
	cpuStatPath    = "/sys/fs/cgroup/cpu.stat"
	cpuMaxPath     = "/sys/fs/cgroup/cpu.max"
	memCurrentPath = "/sys/fs/cgroup/memory.current"
	memMaxPath     = "/sys/fs/cgroup/memory.max"
)

// Usage struct to hold CPU & Memory metrics
type Usage struct {
	CPUUsageMillicores float64 // CPU usage in mCPU
	MemoryUsageMiB     float64 // Memory usage in MiB
	CPULimitMillicores float64 // CPU limit in mCPU (if set)
	MemLimitMiB        float64 // Memory limit in MiB (if set)
	CPUUsageSeconds    float64 // CPU usage in seconds
}

// Read an integer value from a file, given a field name
func keyReadIntFromFile(path, field string) (int64, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return 0, err
	}
	lines := strings.Split(string(data), "\n")
	for _, line := range lines {
		if strings.HasPrefix(line, field) {
			fields := strings.Fields(line)
			if len(fields) < 2 {
				continue
			}
			return strconv.ParseInt(fields[1], 10, 64)
		}
	}
	return 0, fmt.Errorf("%s not found in %s", field, path)
}

func readIntFromFile(path string) (int64, error) {
	str, err := readStringFromFile(path)
	if err != nil {
		return -1, err
	}
	return strconv.ParseInt(str, 10, 64)
}

// Read a string from a file
func readStringFromFile(path string) (string, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return "", err
	}
	return strings.TrimSpace(string(data)), nil
}

// Get CPU usage in millicores (mCPU) and seconds
func getCPUUsage(previousUsage int64, prevTime time.Time) (float64, float64, int64, time.Time, error) {
	usageUsec, err := keyReadIntFromFile(cpuStatPath, "usage_usec")
	if err != nil {
		return 0, 0, 0, time.Now(), err
	}

	// Convert `usage_usec` to seconds
	cpuUsageSeconds := float64(usageUsec) / (100 * 1e6)

	// Get elapsed time
	now := time.Now()
	elapsedTime := now.Sub(prevTime).Seconds()
	if previousUsage > 0 && elapsedTime > 0 {
		// Compute CPU usage delta
		usageDelta := usageUsec - previousUsage
		cpuUsage := (float64(usageDelta) / (elapsedTime * 1000)) // Convert to millicores
		return cpuUsage, cpuUsageSeconds, usageUsec, now, nil
	}

	// If first run, return 0 usage and store the timestamp
	return 0, cpuUsageSeconds, usageUsec, now, nil
}

// Get CPU limits (if defined)
func getCPULimits() (float64, error) {
	data, err := readStringFromFile(cpuMaxPath)
	if err != nil {
		return 0, err
	}
	parts := strings.Fields(data)
	if parts[0] == "max" {
		return -1, nil // No limit
	}
	quota, err := strconv.ParseInt(parts[0], 10, 64)
	if err != nil {
		return 0, err
	}
	period, err := strconv.ParseInt(parts[1], 10, 64)
	if err != nil {
		return 0, err
	}

	// Correct formula for CPU limit in millicores
	return (float64(quota) / float64(period)) * 1000, nil
}

// Get memory usage in MiB
func getMemoryUsage() (float64, error) {
	memBytes, err := readIntFromFile(memCurrentPath)
	if err != nil {
		return 0, err
	}
	return float64(memBytes) / (1024 * 1024), nil // Convert bytes to MiB
}

// Get memory limits (if defined)
func getMemoryLimit() (float64, error) {
	memLimitStr, err := readStringFromFile(memMaxPath)
	if err != nil {
		return 0, err
	}
	if memLimitStr == "max" {
		return -1, nil // No limit
	}
	memBytes, err := strconv.ParseInt(memLimitStr, 10, 64)
	if err != nil {
		return 0, err
	}
	return float64(memBytes) / (1024 * 1024), nil // Convert bytes to MiB
}

// Convert CPU usage seconds to `MM:SS` format (like `ps` command)
func formatCPUTime(seconds float64) string {
	minutes := int(seconds) / 60
	secs := int(seconds) % 60
	return fmt.Sprintf("%02d:%02d", minutes, secs)
}

// Retrieve all usage metrics
func getUsage(previousUsage int64, prevTime time.Time) (*Usage, int64, time.Time, error) {
	// Get CPU usage
	cpuUsage, cpuUsageSeconds, newUsage, newTime, err := getCPUUsage(previousUsage, prevTime)
	if err != nil {
		return nil, previousUsage, prevTime, err
	}

	// Get CPU limit
	cpuLimit, err := getCPULimits()
	if err != nil {
		return nil, previousUsage, prevTime, err
	}

	// Get memory usage
	memUsage, err := getMemoryUsage()
	if err != nil {
		return nil, previousUsage, prevTime, err
	}

	// Get memory limit
	memLimit, err := getMemoryLimit()
	if err != nil {
		return nil, previousUsage, prevTime, err
	}

	// Create Usage object
	return &Usage{
		CPUUsageMillicores: cpuUsage,
		CPULimitMillicores: cpuLimit,
		MemoryUsageMiB:     memUsage,
		MemLimitMiB:        memLimit,
		CPUUsageSeconds:    cpuUsageSeconds,
	}, newUsage, newTime, nil
}

func main() {
	var previousUsage int64 = 0
	var prevTime time.Time = time.Now()

	for {
		// Get usage metrics
		usage, newUsage, newTime, err := getUsage(previousUsage, prevTime)
		if err != nil {
			fmt.Println("Error retrieving usage metrics:", err)
			return
		}

		// Print CPU usage
		fmt.Printf("CPU Usage: %.2f mCPU\n", usage.CPUUsageMillicores)
		fmt.Printf("CPU Usage Time (ps format): %s\n", formatCPUTime(usage.CPUUsageSeconds))
		if usage.CPULimitMillicores > 0 {
			fmt.Printf("CPU Limit: %.2f mCPU\n", usage.CPULimitMillicores)
			fmt.Printf("CPU Usage (%% of Limit): %.2f%%\n", (usage.CPUUsageMillicores/usage.CPULimitMillicores)*100)
		} else {
			fmt.Println("CPU Limit: Not set")
		}

		// Print Memory usage
		fmt.Printf("Memory Usage: %.2f MiB\n", usage.MemoryUsageMiB)
		if usage.MemLimitMiB > 0 {
			fmt.Printf("Memory Limit: %.2f MiB\n", usage.MemLimitMiB)
			fmt.Printf("Memory Usage (%% of Limit): %.2f%%\n", (usage.MemoryUsageMiB/usage.MemLimitMiB)*100)
		} else {
			fmt.Println("Memory Limit: Not set")
		}

		// Update previous usage values
		previousUsage = newUsage
		prevTime = newTime

		// Sleep before next measurement
		time.Sleep(5 * time.Second)
	}
}
